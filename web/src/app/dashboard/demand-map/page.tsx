...
export default function DemandMapPage() {
	...
	const { data: analysis, isLoading } = useQuery({
		queryKey: ["demand-map", "analysis"],
		queryFn: () => api.demandMap.analysis(),
	});
	...
	const buildMutation = useMutation({
		mutationFn: () => api.demandMap.buildMatrix(),
		onSuccess: (data) => {
			queryClient.invalidateQueries({ queryKey: ["demand-map", "analysis"] });
			toast.success(
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground text-xs uppercase tracking-widest">
						Analyze student distribution and geographic clusters
					</p>
				</div>
				<Button
					className="gap-2"
