...
class OptimizerService:
    ...
    async def _do_optimize(
        ...
        logger.info(f"Starting optimization with scenario={scenario_type}, semester={semester}, deadline={arrival_deadline}")
        ...
        logger.info(f"Loaded {len(stops_with_demand)} stops with {total_students} total students")
        ...
        logger.info(f"Loaded {len(buses)} buses with {total_capacity} total capacity")
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "details": {
                        ...
                        "additional_buses_needed": (total_students - total_capacity + 49) // 50  # Assuming 50 capacity per bus
                    },
                    "suggestions": [
        ...
        logger.info(f"Starting CVRPTW solver with timeout={self.config.optimization_timeout_sec}s...")
        t0 = time.time()
        ...
        logger.info(f"CVRPTW solve completed in {model_build_time:.2f}s, generated {len(routes)} routes")
        ...
        logger.info(f"Solution created: {len(routes)} routes, {stats.get('total_students_assigned', 0)} students, ₹{cost_estimate:.2f}")
    ...
    def _solve_cvrptw(
        ...
        """
        Build and solve CVRPTW model using OR-Tools.
        
        Model Structure:
        - Nodes: Campus (0), Stops (1..n), Depots (not nodes, just start locations)
        - Vehicles: Each bus is a vehicle
        - Demands: Student counts at stops
        - Time Windows: Campus must be visited by arrival_deadline
        - Capacity: Bus capacity constraints
        """
        
        logger.info("Starting CVRPTW solver setup...")
        ...
        logger.info(f"Matrix has {len(stop_ids_in_matrix)} stops, {len(stops_with_demand)} with demand")
        ...
        logger.info(f"Valid stops in matrix: {len(valid_stops)}")
        ...
        # Build node mapping
        # Node 0: Campus (virtual depot/end location)
        # Nodes 1..n: Stops
        campus_index = 0
        ...
        # Campus connections (use large penalty to force routes to end there)
        CAMPUS_TIME_PENALTY = 1000
        for i in range(num_nodes):
            time_matrix[i][campus_index] = CAMPUS_TIME_PENALTY
            time_matrix[campus_index][i] = 0
            distance_matrix[i][campus_index] = 0
            distance_matrix[campus_index][i] = 0
        
        # Demands (students at each stop)
        demands = [0]  # Campus has no demand
        for i in range(1, num_nodes):
            stop_data = index_to_stop[i]
            demands.append(stop_data["student_count"])
        
        # Vehicle data
        num_vehicles = len(buses)
        vehicle_capacities = [b["capacity"] for b in buses]
        
        # Vehicle start locations (all start at campus for now)
        depot_index = 0
        ...
        time_windows = [(0, deadline_minutes)]
        for _ in range(num_stops):
            time_windows.append((0, deadline_minutes))
        ...
        logger.info(f"Starting OR-Tools solve with timeout={timeout}s, nodes={num_nodes}, vehicles={num_vehicles}")
        ...
        if not solution:
            logger.error(f"Solver returned no solution after {solve_time:.2f}s")
            raise HTTPException(
                status_code=422,
                detail={
        ...
        logger.info(f"Solver found solution with objective value: {solution.ObjectiveValue()}")
        ...
        for vehicle_id in range(num_vehicles):
            ...
            while not routing.IsEnd(index):
                ...
                if node_index != campus_index and node_index in index_to_stop:
                    ...
                    # Format as HH:MM
                    arrival_str = self._minutes_to_time(arrival_time)
            ...
            # Check if bus actually visited any stops
            if len(route_stops) > 0:
                # Calculate utilization
                utilization = (route_students / bus["capacity"]) * 100
                
                # Check warnings
                if route_time > max_ride_time:
                    route_warnings.append(f"Route exceeds max ride time ({route_time} > {max_ride_time} min)")
                    global_warnings.append(f"Bus {bus['bus_no']}: Ride time {route_time}min exceeds limit")
